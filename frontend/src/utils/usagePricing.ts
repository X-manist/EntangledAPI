...
export function formatTokenPricePerMillion(
  ...
  const amountMultiplier = Number.isFinite(options.amountMultiplier)
    ? Number(options.amountMultiplier)
    : 1
  const formatted = (pricePerMillion * amountMultiplier).toFixed(fractionDigits)
