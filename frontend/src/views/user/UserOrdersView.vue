<template>
  <AppLayout>
    <div class="space-y-4">
      ...
      <!-- Table -->
      <OrderTable :orders="orders" :loading="loading" display-user-currency>
...
function formatRefundTargetAmount(order: PaymentOrder): string {
  if (order.order_type === 'balance') {
    return formatBackendUserAmount(order.amount, 2)
  }
  return formatPaymentAmount(order.amount, order.currency || 'CNY')
}
