          ...
          <div class="flex items-center gap-3">
            ...
            <div class="min-w-0 flex-1">
              ...
              <p class="text-xl font-bold text-green-600 dark:text-green-400">
                {{ formatBackendUserAmount(usageStats?.total_actual_cost || 0, 4) }}
              </p>
              <p class="text-xs text-gray-500 dark:text-gray-400">
                {{ t('usage.actualCost') }} /
                <span class="line-through">{{ formatBackendUserAmount(usageStats?.total_cost || 0, 4) }}</span>
          ...
          <template #cell-cost="{ row }">
            <div class="flex items-center gap-1.5 text-sm">
              <span class="font-medium text-green-600 dark:text-green-400">
                {{ formatBackendUserAmount(row.actual_cost, 6) }}
              </span>
              <!-- Cost Detail Tooltip -->
              <div
                class="group relative"
        ...
        <div class="space-y-1.5">
          ...
          <div class="mb-2 border-b border-gray-700 pb-1.5">
            ...
            <div v-if="tooltipData && tooltipData.input_cost > 0" class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-white">{{ formatBackendUserAmount(tooltipData.input_cost, 6) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.output_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.outputCost') }}</span>
              <span class="font-medium text-white">{{ formatBackendUserAmount(tooltipData.output_cost, 6) }}</span>
            </div>
            ...
            <template v-if="tooltipData && isImageUsage(tooltipData)">
              ...
              <div class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-sky-300">{{ formatBackendUserAmount(imageUnitPrice(tooltipData), 6) }}</span>
              </div>
              <div class="flex items-center justify-between gap-4">
                <span class="text-gray-400">{{ t('usage.imageTotalPrice') }}</span>
                <span class="font-medium text-white">{{ formatBackendUserAmount(tooltipData.total_cost || 0, 6) }}</span>
              </div>
            </template>
            ...
            <template v-else-if="!getDisplayBillingMode(tooltipData) || getDisplayBillingMode(tooltipData) === BILLING_MODE_TOKEN">
              <div v-if="tooltipData && tooltipData.input_tokens > 0" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-sky-300">{{ formatUserTokenPricePerMillion(tooltipData.input_cost, tooltipData.input_tokens) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
              <div v-if="tooltipData && tooltipData.output_tokens > 0" class="flex items-center justify-between gap-4">
                <span class="text-gray-400">{{ t('usage.outputTokenPrice') }}</span>
                <span class="font-medium text-violet-300">{{ formatUserTokenPricePerMillion(tooltipData.output_cost, tooltipData.output_tokens) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
            </template>
            <div v-else class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('usage.unitPrice') }}</span>
              <span class="font-medium text-sky-300">{{ formatBackendUserAmount(tooltipData?.total_cost || 0, 6) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.cache_creation_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.cacheCreationCost') }}</span>
              <span class="font-medium text-white">{{ formatBackendUserAmount(tooltipData.cache_creation_cost, 6) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.cache_read_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.cacheReadCost') }}</span>
              <span class="font-medium text-white">{{ formatBackendUserAmount(tooltipData.cache_read_cost, 6) }}</span>
            </div>
          </div>
          <!-- Rate and Summary -->
          ...
          <div class="flex items-center justify-between gap-6 border-t border-gray-700 pt-1.5">
            ...
            <span class="font-semibold text-green-400"
              >{{ formatBackendUserAmount(tooltipData?.actual_cost || 0, 6) }}</span
            >
          </div>
        </div>
        <!-- Tooltip Arrow (left side) -->
...
const userAmountMultiplier = computed(() => 1 / balanceRechargeMultiplier.value)

function formatUserTokenPricePerMillion(
  cost: number | null | undefined,
  tokens: number | null | undefined
): string {
  return formatTokenPricePerMillion(cost, tokens, {
    symbol: userCurrencySymbol,
    amountMultiplier: userAmountMultiplier.value
  })
}
