        ...
        <template v-else>
          ...
          <template v-if="activeTab === 'recharge'">
            ...
            <div class="card p-5">
              ...
              <p class="mt-0.5 text-sm font-medium text-green-600 dark:text-green-400">{{ t('payment.currentBalance') }}: {{ formatBackendUserAmount(user?.balance || 0, 2) }}</p>
            </div>
            ...
            <div v-if="validAmount > 0" class="card p-6">
              <div class="space-y-2 text-sm">
                ...
                <div v-if="balanceRechargeMultiplier !== 1" class="flex justify-between" :class="{ 'border-t border-gray-200 pt-2 dark:border-dark-600': feeRate <= 0 }">
                  ...
                  <span class="text-gray-900 dark:text-white">{{ formatBackendUserAmount(creditedAmount, 2) }}</span>
                </div>
                <p v-if="balanceRechargeMultiplier !== 1" class="border-t border-gray-200 pt-2 text-xs text-gray-500 dark:border-dark-600 dark:text-gray-400">
                  {{ t('payment.rechargeRatePreview', { cny: userCurrencyPerCny }) }}
                </p>
              </div>
            </div>
              ...
              <div class="card p-5">
                ...
                <div class="mt-3 grid grid-cols-2 gap-3">
                  ...
                  <div v-if="selectedPlan.daily_limit_usd != null">
                    ...
                    <div class="text-lg font-semibold text-gray-800 dark:text-gray-200">{{ formatBackendUserAmount(selectedPlan.daily_limit_usd, 2) }}</div>
                  </div>
                  <div v-if="selectedPlan.weekly_limit_usd != null">
                    <span class="text-xs text-gray-400 dark:text-gray-500">{{ t('payment.planCard.weeklyLimit') }}</span>
                    <div class="text-lg font-semibold text-gray-800 dark:text-gray-200">{{ formatBackendUserAmount(selectedPlan.weekly_limit_usd, 2) }}</div>
                  </div>
                  <div v-if="selectedPlan.monthly_limit_usd != null">
                    <span class="text-xs text-gray-400 dark:text-gray-500">{{ t('payment.planCard.monthlyLimit') }}</span>
                    <div class="text-lg font-semibold text-gray-800 dark:text-gray-200">{{ formatBackendUserAmount(selectedPlan.monthly_limit_usd, 2) }}</div>
                  </div>
...
import { setUserCurrencyMultiplier, useUserCurrency } from '@/composables/useUserCurrency'
...
const { formatBackendUserAmount } = useUserCurrency()
...
const userCurrencyPerCny = computed(() => formatBackendUserAmount(balanceRechargeMultiplier.value, 2))
...
onMounted(async () => {
  try {
    ...
    setUserCurrencyMultiplier(res.data.balance_recharge_multiplier)
