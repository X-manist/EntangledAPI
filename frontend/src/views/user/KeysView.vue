<template>
  <AppLayout>
            ...
            <div class="text-sm">
              <div class="flex items-center gap-1.5">
                ...
                <span class="font-medium text-gray-900 dark:text-white">
                  {{ formatBackendUserAmount(usageStats[row.id]?.today_actual_cost ?? 0, 4) }}
                </span>
              </div>
              <div class="mt-0.5 flex items-center gap-1.5">
                <span class="text-gray-500 dark:text-gray-400">{{ t('keys.total') }}:</span>
                <span class="font-medium text-gray-900 dark:text-white">
                  {{ formatBackendUserAmount(usageStats[row.id]?.total_actual_cost ?? 0, 4) }}
                </span>
              </div>
              ...
              <div v-if="row.quota > 0" class="mt-1.5">
                <div class="flex items-center gap-1.5">
                  ...
                  <span :class="[
                    ...
                    {{ formatBackendUserAmount(row.quota_used || 0, 2) }} / {{ formatBackendUserAmount(row.quota || 0, 2) }}
                  </span>
                </div>
              ...
              <div v-if="row.rate_limit_5h > 0">
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span :class="[
                    ...
                    {{ formatBackendUserAmount(row.usage_5h || 0, 2) }}/{{ formatBackendUserAmount(row.rate_limit_5h || 0, 2) }}
                  </span>
                </div>
              ...
              <div v-if="row.rate_limit_1d > 0">
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span :class="[
                    ...
                    {{ formatBackendUserAmount(row.usage_1d || 0, 2) }}/{{ formatBackendUserAmount(row.rate_limit_1d || 0, 2) }}
                  </span>
                </div>
                <div class="h-1 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                  <div
              ...
              <div v-if="row.rate_limit_7d > 0">
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span :class="[
                    ...
                    {{ formatBackendUserAmount(row.usage_7d || 0, 2) }}/{{ formatBackendUserAmount(row.rate_limit_7d || 0, 2) }}
                  </span>
                </div>
                <div class="h-1 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                  <div
      ...
      <form id="key-form" @submit.prevent="handleSubmit" class="space-y-5">
        ...
        <div class="space-y-3">
          ...
          <div class="space-y-4">
            <div>
              <div class="relative">
                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">{{ userCurrencySymbol }}</span>
                <input
            ...
            <div v-if="showEditModal && selectedKey && selectedKey.quota > 0">
              ...
              <div class="flex items-center gap-2">
                <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700">
                  <span class="font-medium text-gray-900 dark:text-white">
                    {{ formatBackendUserAmount(selectedKey.quota_used || 0, 4) }}
                  </span>
                  <span class="mx-2 text-gray-400">/</span>
                  <span class="text-gray-500 dark:text-gray-400">
                    {{ formatBackendUserAmount(selectedKey.quota || 0, 2) }}
                  </span>
                </div>
                <button
                  type="button"
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700 text-sm">
                    <span :class="[
                      ...
                      {{ formatBackendUserAmount(selectedKey.usage_5h || 0, 4) }}
                    </span>
                    <span class="mx-2 text-gray-400">/</span>
                    <span class="text-gray-500 dark:text-gray-400">
                      {{ formatBackendUserAmount(selectedKey.rate_limit_5h || 0, 2) }}
                    </span>
                  </div>
                </div>
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700 text-sm">
                    <span :class="[
                      ...
                      {{ formatBackendUserAmount(selectedKey.usage_1d || 0, 4) }}
                    </span>
                    <span class="mx-2 text-gray-400">/</span>
                    <span class="text-gray-500 dark:text-gray-400">
                      {{ formatBackendUserAmount(selectedKey.rate_limit_1d || 0, 2) }}
                    </span>
                  </div>
                </div>
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700 text-sm">
                    <span :class="[
                      ...
                      {{ formatBackendUserAmount(selectedKey.usage_7d || 0, 4) }}
                    </span>
                    <span class="mx-2 text-gray-400">/</span>
                    <span class="text-gray-500 dark:text-gray-400">
                      {{ formatBackendUserAmount(selectedKey.rate_limit_7d || 0, 2) }}
                    </span>
                  </div>
                </div>
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
    ...
    <ConfirmDialog
      ...
      :message="t('keys.resetQuotaConfirmMessage', { name: selectedKey?.name, used: formatBackendUserAmount(selectedKey?.quota_used || 0, 4) })"
...
const {
  backendAmountToUserAmount,
  formatBackendUserAmount,
  loadUserCurrencyConfig,
  userAmountToBackendAmount,
  userCurrencySymbol
} = useUserCurrency()
...
const editKey = (key: ApiKey) => {
  ...
  formData.value = {
    ...
    quota: key.quota > 0 ? backendAmountToUserAmount(key.quota) : null,
    enable_rate_limit: (key.rate_limit_5h > 0) || (key.rate_limit_1d > 0) || (key.rate_limit_7d > 0),
    rate_limit_5h: key.rate_limit_5h > 0 ? backendAmountToUserAmount(key.rate_limit_5h) : null,
    rate_limit_1d: key.rate_limit_1d > 0 ? backendAmountToUserAmount(key.rate_limit_1d) : null,
    rate_limit_7d: key.rate_limit_7d > 0 ? backendAmountToUserAmount(key.rate_limit_7d) : null,
...
const handleSubmit = async () => {
  ...
  const quota = formData.value.quota && formData.value.quota > 0
    ? userAmountToBackendAmount(formData.value.quota)
    : 0
  ...
  const rateLimitData = formData.value.enable_rate_limit ? {
    rate_limit_5h: formData.value.rate_limit_5h && formData.value.rate_limit_5h > 0 ? userAmountToBackendAmount(formData.value.rate_limit_5h) : 0,
    rate_limit_1d: formData.value.rate_limit_1d && formData.value.rate_limit_1d > 0 ? userAmountToBackendAmount(formData.value.rate_limit_1d) : 0,
    rate_limit_7d: formData.value.rate_limit_7d && formData.value.rate_limit_7d > 0 ? userAmountToBackendAmount(formData.value.rate_limit_7d) : 0,
...
const executeCcsImport = (row: ApiKey, clientType: CcSwitchClientType) => {
  ...
  const providerName = resolveDisplayBrandName(publicSettings.value?.site_name, locale.value)
