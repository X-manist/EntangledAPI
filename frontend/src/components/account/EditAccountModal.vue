<template>
  ...
  >
    ...
    >
          ...
          <input
            ...
            :placeholder="
              account.platform === 'openai'
                ...
                : account.platform === 'github-copilot'
      ...
      <div
        v-if="account.platform === 'github-copilot' && account.type === 'oauth'"
      ...
      >
        <div>
          ...
          <input
            ...
            type="text"
            class="input"
            data-testid="copilot-oauth-base-url"
        ...
        <div>
          ...
          <input
            v-model="editGitHubCopilotToken"
            type="password"
            class="input font-mono"
            data-testid="copilot-oauth-github-token"
            autocomplete="new-password"
            data-1p-ignore
            data-lpignore="true"
            data-bwignore="true"
          ...
          />
          <p class="input-hint">
            {{ t('admin.accounts.leaveEmptyToKeep') }} {{ githubCopilotOAuthTokenHint }}
          </p>
        </div>
      </div>
...
const baseUrlHint = computed(() => {
  ...
  if (props.account.platform === 'github-copilot') return 'GitHub Copilot API base URL, usually https://api.githubcopilot.com'
...
const editGitHubCopilotToken = ref('')
// Bedrock credentials
...
const defaultBaseUrl = computed(() => {
  ...
  if (props.account?.platform === 'github-copilot') return 'https://api.githubcopilot.com'
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
    ...
    const platformDefaultUrl =
      newAccount.platform === 'openai'
        ? 'https://api.openai.com'
        : newAccount.platform === 'github-copilot'
          ? 'https://api.githubcopilot.com'
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.type === 'apikey') {
      ...
      updatePayload.credentials = newCredentials
    } else if (props.account.platform === 'github-copilot' && props.account.type === 'oauth') {
      const currentCredentials = (props.account.credentials as Record<string, unknown>) || {}
      const newCredentials: Record<string, unknown> = {
        ...currentCredentials,
      ...
      if (editGitHubCopilotToken.value.trim()) {
        if (isGitHubPersonalAccessToken(editGitHubCopilotToken.value)) {
          appStore.showError(githubCopilotPATError)
          return
        }
        newCredentials.github_token = editGitHubCopilotToken.value.trim()
        delete newCredentials.access_token
        delete newCredentials.expires_at
        delete newCredentials.copilot_sku
      } else if (!currentCredentials.github_token) {
        appStore.showError('GitHub token is required')
        return
      }
