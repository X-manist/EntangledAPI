  ...
  >
    <div v-if="account" class="space-y-4">
      ...
      <div
        ...
        class="space-y-4 rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900/20"
      >
        <div>
          <label class="input-label">{{ t('admin.accounts.baseUrl') }}</label>
          <input
            v-model="copilotBaseUrl"
            type="text"
            class="input"
            placeholder="https://api.githubcopilot.com"
          />
          <p class="input-hint">GitHub Copilot API base URL, usually https://api.githubcopilot.com</p>
        </div>
        <div>
          ...
          />
          <p class="input-hint">{{ githubCopilotOAuthTokenHint }}</p>
        </div>
        <p v-if="copilotError" class="text-sm text-red-600 dark:text-red-400">{{ copilotError }}</p>
      </div>
    ...
    <template #footer>
      <div v-if="account" class="flex justify-between gap-3">
        ...
        <button
          v-if="isGitHubCopilot"
          type="button"
          :disabled="copilotLoading || !copilotGitHubToken.trim()"
          class="btn btn-primary"
          @click="handleSaveCopilotToken"
        >
          <svg
            v-if="copilotLoading"
          ...
          {{ copilotLoading ? t('admin.accounts.oauth.verifying') : t('admin.accounts.oauth.completeAuth') }}
        </button>
        <button
...
const copilotBaseUrl = ref('https://api.githubcopilot.com')
...
const isGitHubCopilot = computed(() => props.account?.platform === 'github-copilot')
  ...
  (newVal) => {
    if (newVal && props.account) {
      ...
      if (isGitHubCopilot.value) {
        const creds = (props.account.credentials || {}) as Record<string, unknown>
        copilotBaseUrl.value = typeof creds.base_url === 'string' && creds.base_url.trim()
          ? creds.base_url.trim()
...
const resetState = () => {
  ...
  copilotBaseUrl.value = 'https://api.githubcopilot.com'
  copilotGitHubToken.value = ''
...
const handleSaveCopilotToken = async () => {
  ...
  if (!copilotGitHubToken.value.trim()) {
    copilotError.value = 'GitHub token is required'
    return
  }
  if (isGitHubPersonalAccessToken(copilotGitHubToken.value)) {
    copilotError.value = githubCopilotPATError
    return
  }
  ...
  try {
    ...
    copilotError.value = error.response?.data?.detail || 'Failed to update Copilot token'
    appStore.showError(copilotError.value)
  } finally {
    copilotLoading.value = false
  }
}
