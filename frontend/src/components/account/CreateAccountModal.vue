    ...
    >
      ...
      <div>
        ...
        <div class="mt-2 flex rounded-lg bg-gray-100 p-1 dark:bg-dark-700" data-tour="account-form-platform">
          ...
          <button
            type="button"
            @click="form.platform = 'github-copilot'"
            :class="[
              'flex flex-1 items-center justify-center gap-2 rounded-md px-4 py-2.5 text-sm font-medium transition-all',
      ...
      <!-- Account Type Selection (GitHub Copilot) -->
      <div v-if="form.platform === 'github-copilot'">
        <label class="input-label">{{ t('admin.accounts.accountType') }}</label>
        <div class="mt-2 grid grid-cols-2 gap-3" data-tour="account-form-type">
          <button
            type="button"
          ...
          >
            ...
            <div>
              <span class="block text-sm font-medium text-gray-900 dark:text-white">GitHub OAuth Token</span>
              <span class="text-xs text-gray-500 dark:text-gray-400">Exchange a Copilot session token on demand</span>
            </div>
          </button>
          ...
          >
            ...
            <div>
              <span class="block text-sm font-medium text-gray-900 dark:text-white">Access Token</span>
              <span class="text-xs text-gray-500 dark:text-gray-400">Store a Copilot bearer/session token directly</span>
            </div>
          </button>
        </div>
      </div>
      ...
      <div v-if="form.platform === 'github-copilot' && accountCategory === 'oauth-based'" class="space-y-4">
        <div>
        ...
        <div>
          <label class="input-label">GitHub OAuth Token</label>
          <input
            v-model="githubCopilotTokenValue"
            type="password"
            required
            class="input font-mono"
            placeholder="ghu_... / gho_..."
          />
          <p class="input-hint">
            {{ githubCopilotOAuthTokenHint }}
          </p>
        </div>
      </div>
      ...
      <div v-if="form.type === 'apikey' && form.platform !== 'antigravity'" class="space-y-4">
        ...
        <div>
          <label class="input-label">
            {{ form.platform === 'github-copilot' ? 'Access Token / Session Token' : t('admin.accounts.apiKeyRequired') }}
          </label>
          <input
            v-model="apiKeyValue"
            type="password"
...
import {
  githubCopilotOAuthTokenHint,
  githubCopilotPATError,
  isGitHubPersonalAccessToken
} from '@/utils/githubCopilotToken'
import {
...
const oauthStepTitle = computed(() => {
  ...
  if (form.platform === 'github-copilot') return 'GitHub Copilot'
...
const baseUrlHint = computed(() => {
  ...
  if (form.platform === 'github-copilot') return 'GitHub Copilot API base URL, usually https://api.githubcopilot.com'
...
const apiKeyHint = computed(() => {
  ...
  if (form.platform === 'github-copilot') return 'Paste a GitHub Copilot access token or session token.'
...
const githubCopilotTokenValue = ref('')
...
const isOAuthFlow = computed(() => {
  if (form.platform === 'github-copilot') {
    return false
  }
...
watch(
  () => form.platform,
  (newPlatform) => {
    // OpenAI account creation should always land on the legacy OAuth flow by
    // default so the auth URL/code screen is one "Next" away after switching.
    ...
    apiKeyBaseUrl.value =
      (newPlatform === 'openai')
        ? 'https://api.openai.com'
        : newPlatform === 'github-copilot'
          ? 'https://api.githubcopilot.com'
        : newPlatform === 'gemini'
          ? 'https://generativelanguage.googleapis.com'
          : 'https://api.anthropic.com'
    githubCopilotTokenValue.value = ''
...
const handleSubmit = async () => {
  ...
  if (form.platform === 'github-copilot' && accountCategory.value === 'oauth-based') {
    if (!form.name.trim()) {
      appStore.showError(t('admin.accounts.pleaseEnterAccountName'))
      return
    }
    if (!githubCopilotTokenValue.value.trim()) {
      appStore.showError('Please enter a GitHub token')
      return
    }
    if (isGitHubPersonalAccessToken(githubCopilotTokenValue.value)) {
      appStore.showError(githubCopilotPATError)
      return
    }

    const credentials: Record<string, unknown> = {
      base_url: apiKeyBaseUrl.value.trim() || 'https://api.githubcopilot.com',
      github_token: githubCopilotTokenValue.value.trim()
    }
    ...
    await createAccountAndFinish('github-copilot', 'oauth', credentials)
    return
  }
