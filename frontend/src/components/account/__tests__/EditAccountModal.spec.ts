...
function buildCopilotOAuthAccount() {
  return {
    id: 2,
    name: 'Copilot OAuth',
    notes: '',
    ...
    type: 'oauth',
    credentials: {
      github_token: 'github_pat_old',
      access_token: 'copilot_session_old',
...
describe('EditAccountModal', () => {
  ...
  it('updates Copilot OAuth credentials with a fresh GitHub token', async () => {
    const account = buildCopilotOAuthAccount()
    updateAccountMock.mockReset()
    checkMixedChannelRiskMock.mockReset()
    checkMixedChannelRiskMock.mockResolvedValue({ has_risk: false })
    updateAccountMock.mockResolvedValue(account)
    ...
    await wrapper.get('[data-testid="copilot-oauth-base-url"]').setValue('https://proxy.githubcopilot.example')
    await wrapper.get('[data-testid="copilot-oauth-github-token"]').setValue('github_pat_new')
    await wrapper.get('form#edit-account-form').trigger('submit.prevent')

    expect(updateAccountMock).toHaveBeenCalledTimes(1)
    expect(updateAccountMock.mock.calls[0]?.[1]?.credentials).toMatchObject({
      base_url: 'https://proxy.githubcopilot.example',
      github_token: 'github_pat_new',
      model_mapping: {
        'gpt-4.1': 'gpt-4.1'
      }
    })
    expect(updateAccountMock.mock.calls[0]?.[1]?.credentials).not.toHaveProperty('access_token')
    expect(updateAccountMock.mock.calls[0]?.[1]?.credentials).not.toHaveProperty('expires_at')
  })
