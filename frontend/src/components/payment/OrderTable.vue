...
function formatGatewayAmount(value: number, currency?: string | null): string {
  return formatPaymentAmount(value, currency || 'CNY')
}

function formatCreditedAmount(row: PaymentOrder): string {
  if (row.order_type === 'balance') {
    return props.displayUserCurrency
      ? formatBackendUserAmount(row.amount, 2)
      : `$${row.amount.toFixed(2)}`
  }
  return formatGatewayAmount(row.amount, row.currency)
}

onMounted(() => {
  if (props.displayUserCurrency) {
