      ...
      <template v-if="notifyEnabled">
        ...
        <div>
          ...
          <div class="flex items-center gap-2">
            <span class="text-gray-500">{{ userCurrencySymbol }}</span>
            <input
              ...
              step="0.01"
              class="input flex-1"
              :placeholder="systemDefaultThreshold > 0 ? `${t('profile.balanceNotify.systemDefault')} ${formatBackendUserAmount(systemDefaultThreshold, 2)}` : t('profile.balanceNotify.thresholdPlaceholder')"
            />
            <button
...
const customThreshold = ref<number | null>(
  props.threshold != null ? backendAmountToUserAmount(props.threshold) : null
)
...
watch([() => props.threshold, balanceRechargeMultiplier], ([threshold]) => {
  customThreshold.value = threshold != null ? backendAmountToUserAmount(threshold) : null
})
...
const handleThresholdUpdate = async () => {
  ...
  try {
    const threshold = customThreshold.value && customThreshold.value > 0
      ? userAmountToBackendAmount(customThreshold.value)
      : 0
