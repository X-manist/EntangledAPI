...
type WorkspaceQuery struct {
	...
	withKnowledgeFiles *KnowledgeFileQuery
...
func (_q *WorkspaceQuery) QueryConversations() *ConversationQuery {
...
// QueryKnowledgeFiles chains the current query on the "knowledge_files" edge.
func (_q *WorkspaceQuery) QueryKnowledgeFiles() *KnowledgeFileQuery {
	query := (&KnowledgeFileClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *WorkspaceQuery) Clone() *WorkspaceQuery {
	...
	return &WorkspaceQuery{
		...
		withKnowledgeFiles: _q.withKnowledgeFiles.Clone(),
...
func (_q *WorkspaceQuery) WithConversations(opts ...func(*ConversationQuery)) *WorkspaceQuery {
	query := (&ConversationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithKnowledgeFiles tells the query-builder to eager-load the nodes that are connected to
// the "knowledge_files" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *WorkspaceQuery) WithKnowledgeFiles(opts ...func(*KnowledgeFileQuery)) *WorkspaceQuery {
	query := (&KnowledgeFileClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withKnowledgeFiles = query
	return _q
}
...
func (_q *WorkspaceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Workspace, error) {
	...
	if query := _q.withConversations; query != nil {
		if err := _q.loadConversations(ctx, query, nodes,
			func(n *Workspace) { n.Edges.Conversations = []*Conversation{} },
			func(n *Workspace, e *Conversation) { n.Edges.Conversations = append(n.Edges.Conversations, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withKnowledgeFiles; query != nil {
		if err := _q.loadKnowledgeFiles(ctx, query, nodes,
			func(n *Workspace) { n.Edges.KnowledgeFiles = []*KnowledgeFile{} },
			func(n *Workspace, e *KnowledgeFile) { n.Edges.KnowledgeFiles = append(n.Edges.KnowledgeFiles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *WorkspaceQuery) loadConversations(ctx context.Context, query *ConversationQuery, nodes []*Workspace, init func(*Workspace), assign func(*Workspace, *Conversation)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(conversation.FieldWorkspaceID)
	}
	query.Where(predicate.Conversation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(workspace.ConversationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *WorkspaceQuery) loadKnowledgeFiles(ctx context.Context, query *KnowledgeFileQuery, nodes []*Workspace, init func(*Workspace), assign func(*Workspace, *KnowledgeFile)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Workspace)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(knowledgefile.FieldWorkspaceID)
	}
	query.Where(predicate.KnowledgeFile(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(workspace.KnowledgeFilesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
