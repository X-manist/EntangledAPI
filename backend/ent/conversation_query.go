...
func (_q *ConversationQuery) QueryWorkspace() *WorkspaceQuery {
...
func (_q *ConversationQuery) WithWorkspace(opts ...func(*WorkspaceQuery)) *ConversationQuery {
	query := (&WorkspaceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ConversationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Conversation, error) {
	...
	if query := _q.withWorkspace; query != nil {
		if err := _q.loadWorkspace(ctx, query, nodes, nil,
			func(n *Conversation, e *Workspace) { n.Edges.Workspace = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *ConversationQuery) loadWorkspace(ctx context.Context, query *WorkspaceQuery, nodes []*Conversation, init func(*Conversation), assign func(*Conversation, *Workspace)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Conversation)
	for i := range nodes {
...
func (_q *ConversationQuery) loadMessages(ctx context.Context, query *MessageQuery, nodes []*Conversation, init func(*Conversation), assign func(*Conversation, *Message)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(message.FieldConversationID)
	}
...
func (_q *ConversationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withWorkspace != nil {
			_spec.Node.AddColumnOnce(conversation.FieldWorkspaceID)
		}
	}
