...
func (s *OpenAIGatewayService) forwardAsRawChatCompletions(
	...
	if account.IsGitHubCopilot() {
		var err error
		apiKey, err = s.ensureGitHubCopilotAccessToken(ctx, account)
		if err != nil {
			return nil, fmt.Errorf("account %d access token: %w", account.ID, err)
		}
	...
	if account.IsGitHubCopilot() {
		applyGitHubCopilotHeaders(upstreamReq, account, upstreamReq.Header.Get("Accept"))
	}
