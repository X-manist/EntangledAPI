...
func (s *AccountTestService) buildGitHubCopilotUpstreamModelsRequest(ctx context.Context, account *Account) (*http.Request, error) {
	authToken, err := s.ensureGitHubCopilotAccessToken(ctx, account)
	if err != nil {
		return nil, newUpstreamModelSyncConfigError("No GitHub Copilot access token is available", err)
	}
	...
	normalizedBaseURL, err := s.validateUpstreamBaseURL(baseURL)
	if err != nil {
		return nil, newUpstreamModelSyncConfigError("Invalid GitHub Copilot base URL", err)
	}
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, buildOpenAIModelsURL(normalizedBaseURL), nil)
	if err != nil {
