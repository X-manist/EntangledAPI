...
const testGeminiCLIOAuthClientID = "test-gemini-cli-client-id"
...
func TestBuildAuthorizationURL_MissingBuiltinSecret(t *testing.T) {
	setTestGeminiCLIBuiltinOAuth(t, "")
	...
	if err == nil {
		t.Fatal("缺少内置 Gemini CLI client_secret 时应报错")
	}
	if !strings.Contains(err.Error(), GeminiCLIOAuthClientSecretEnv) {
		t.Errorf("错误消息应包含 %s，实际: %v", GeminiCLIOAuthClientSecretEnv, err)
	}
}
...
func TestEffectiveOAuthConfig_NoEnvSecret(t *testing.T) {
	...
	if err == nil {
		t.Fatal("不设置内置 client_secret 时应报错")
	}
	if !strings.Contains(err.Error(), GeminiCLIOAuthClientSecretEnv) {
		t.Errorf("错误消息应包含 %s，实际: %v", GeminiCLIOAuthClientSecretEnv, err)
	}
